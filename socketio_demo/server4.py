...
class AudioBuffer:
    ...
    def add_chunk(self, audio_array):
        ...
        # Debug logging
        #print(f"Current RMS: {volume_rms:.2f}, Avg Volume: {avg_volume:.2f}, Silent: {is_silent}")
        ...
        if not is_silent:
            ...
            if self.silence_start is None:
                ...
                #print(f"Silence detected at {self.silence_start}")
            ...
            # Debug logging
            #print(f"Silence duration: {silence_duration:.2f}s, Total duration: {total_duration:.2f}s")
            
            if silence_duration >= SILENCE_DURATION:
                #print("Saving due to silence duration")
                return self.save_buffer()
            elif total_duration >= MAX_AUDIO_DURATION:
                #print("Saving due to max duration")
                return self.save_buffer()
    ...
    def save_buffer(self):
        if not self.buffer or self.valid_chunk_count < MIN_VALID_CHUNKS:
            #print(f"Buffer rejected: chunks={len(self.buffer)}, valid_chunks={self.valid_chunk_count}")
            self.reset()
            return False
        ...
        #print(f"Actual audio duration: {duration:.2f}s")
        
        if duration < MIN_AUDIO_DURATION:
            #print(f"Audio too short: {duration:.2f}s < {MIN_AUDIO_DURATION}s")
            self.reset()
            return False
            
        if duration > MAX_AUDIO_DURATION:
            #print(f"Audio too long: {duration:.2f}s > {MAX_AUDIO_DURATION}s")
        ...
        if final_rms < SILENCE_THRESHOLD:
            #print(f"Audio too quiet: RMS={final_rms:.2f} < {SILENCE_THRESHOLD}")
            self.reset()
            return False
