...
#         os.remove(file_name)
#         if transcription:
#             sio.emit('transcription_received', {'text': transcription}, room=current_sid)
#             answer = send_to_rag_api(transcription, current_sid)
#             if answer:
#                 sio.emit('translation_received', {'text': answer}, room=current_sid)
...
#                         sio.emit('audio_ready', {'file': file_name}, room=current_sid)
...
#     payload = {"sid": sid}
#     requests.post(reset_url, json=payload)
...
#     eventlet.wsgi.server(eventlet.listen(("0.0.0.0", 8504)), app)
            ...
            if answer:
                ...
                if download_url:
                    ...
                    if audio_response.status_code == 200:
                        ...
                        sio.emit('audio_ready', {'file': file_name}, room=current_sid)
                        end_time=time.time()
                        tts_time_taken=end_time-start_time
