...
def send_to_rag_api(transcription, sid):
    try:
        ...
        response = session.post(TEMP_RAG_API_URL, json=payload)
...
class AudioBuffer:
    ...
    def save_buffer(self):
        ...
        eventlet.spawn(process_audio_parallel, file_name, current_sid)
        
        # Delete the temporary audio file after a delay (allowing time for processing)
        def remove_file():
            eventlet.sleep(4)  # Give time for file to be processed
            try:
                os.remove(file_name)
...
@sio.event
def request_welcome_message(sid):
    welcome_message="Welcome to ChatBot-AI. your voice-powered assistant for seamless document conversations."
    sio.emit('translation_received', {'text': welcome_message}, room=sid)
    download_url = send_to_tts_api(welcome_message)
    if download_url:
        file_name = download_audio_file(download_url, sid)
        if file_name:
            sio.emit('audio_ready', {'file': file_name}, room=sid)
